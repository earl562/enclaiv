...
| Task | Status | Notes |
|------|--------|-------|
| Run pre-built Python unikernel (`kraft run unikraft.org/python3.12`) | ⬜ | |
| Create test-agent directory with Dockerfile + agent.py + Kraftfile | ⬜ | |
| `kraft build` custom unikernel | ⬜ | |
| `kraft run` — see output from inside VM | ⬜ | |
...
| VM traffic routed through proxy | ⬜ | Need to test on Linux box with Unikraft VM |
